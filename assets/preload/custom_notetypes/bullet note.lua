function onCreate()
	...
	for i = 0, getProperty('unspawnNotes.length')-1 do
		if getPropertyFromGroup('unspawnNotes', i, 'noteType') == 'bullet note' then --Check if the note on the chart is a Bullet Note
			setPropertyFromGroup('unspawnNotes', i, 'texture', 'BulletNote'); --Change texture
...
function goodNoteHit(id, direction, noteType, isSustainNote)
	if noteType == 'bullet note' then
		if difficulty == 1 then
			playSound('bulletshoot', 0.5);
		elseif difficulty == 2 then
			playSound('bulletshoot', 0.7);
		end
	...
	function noteMiss(id, direction, noteType, isSustainNote)
		if noteType == 'bullet note' and difficulty == 0 then
		...
		if noteType == 'bullet note' and difficulty == 2 then
			setProperty('health', -2);
			playSound('bulletshoot', 0.5);
			characterPlayAnim('boyfriend', 'hurt', true);
		end
